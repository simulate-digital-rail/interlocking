...
class SUMOInfrastructureProvider(InfrastructureProvider):
    ...
    async def set_signal_state(self, yaramo_signal, target_state):
        if target_state == "go":
            self.traci_instance.setRedYellowGreenState(yaramo_signal.name, "GG")
        elif target_state == "halt":
            if yaramo_signal.direction == SignalDirection.IN:
                self.traci_instance.setRedYellowGreenState(yaramo_signal.name, "rG")
            else:
                self.traci_instance.setRedYellowGreenState(yaramo_signal.name, "Gr")
